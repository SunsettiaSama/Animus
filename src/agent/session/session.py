...
class AgentSession:
    ...
    def reset(self) -> None:
        """清空对话历史并闭合 presence dialogue 会话。"""
        if hasattr(self._conv_loop, "reset"):
            self._conv_loop.reset(session_id=self._id)
        else:
    ...
    def close(self) -> None:
        ...
        if hasattr(self._conv_loop, "close"):
            self._conv_loop.close(session_id=self._id)
        else:
    ...
    def _run_post_process(self) -> None:
        ...
        if hasattr(self._conv_loop, "post_process"):
            self._conv_loop.post_process(session_id=self._id)
        else:
