...
from .channel import HumanEditChannel
from .config import OrchestratorConfig
from .document import PlanDocument, PlanModule, PlanTask, TaskExecutionContext, TaskStatus
from .execution_context import PlanExecutionContext
from .event import (
    HumanPatchEvent,
    LifecycleStateEvent,
    PlanAbortEvent,
...
from .log import PlanLogger
from .patch import HumanPatch, PatchOp
from .planner import PlannerAgent
from .replanner import ReplanDecision, ReplannerAgent
...
class FlowOrchestrator:
    ...
    def _make_step_callback(self, task_id: str, plan_id: str) -> Callable[..., None]:
        loop = getattr(self, "_main_loop", None)

        def schedule_emit_raw(d: dict) -> None:
            if loop is not None and loop.is_running():
                loop.call_soon_threadsafe(self._emit_raw_dict, d)
            else:
                self._emit_raw_dict(d)

        return make_executor_tao_stream_callback(
            plan_id=plan_id,
            task_id=task_id,
            task_steps=self._task_steps,
            schedule_emit_raw=schedule_emit_raw,
        )
