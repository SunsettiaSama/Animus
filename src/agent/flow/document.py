...
class PlanDocument:
    ...
    def get_ready_tasks(self) -> list[PlanTask]:
        specs: list[tuple[str, frozenset[str], NodeStatus]] = [
            (t.task_id, frozenset(t.depends_on), t.status) for t in self.all_tasks()
        ]
        rid = set(ready_node_ids(specs))
        return [t for t in self.all_tasks() if t.task_id in rid]
    ...
    def compute_dag_width(self) -> int:
        ...
        deps = {
            t.task_id: frozenset(d for d in t.depends_on if d in all_ids)
            for t in tasks
        }
        return max_parallel_width(all_ids, deps)
