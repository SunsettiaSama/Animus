...
class SurpriseLauncher:
    ...
    def tick(self) -> bool:
        """累积一次，随机判定是否触发。触发后重置并返回 True，否则返回 False。"""
        self._p = min(1.0, self._p + self._growth_rate)
    ...
    def to_dict(self) -> dict:
        return {
            "p":           self._p,
            "initial_p":   self._initial_p,
            "growth_rate": self._growth_rate,
        }

    @classmethod
    def from_dict(cls, d: dict) -> SurpriseLauncher:
        launcher = cls(
            initial_p=d.get("initial_p", 0.02),
            growth_rate=d.get("growth_rate", 0.03),
        )
        launcher._p = float(d.get("p", launcher._initial_p))
        return launcher
