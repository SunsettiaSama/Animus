...
_FILENAME = "tao_dialogue.jsonl"
_RETENTION_DAYS = 365


class LedgerEventLog:
    """Tao / 交互事件日志（单文件 append-only）。"""
    ...
    def recent(
        ...
        with open(self._path, encoding="utf-8") as f:
            for raw in f:
                ...
                if ts < cutoff:
                    continue
                ev = LedgerEvent.from_dict(d)
                if kinds is not None and ev.kind not in kinds:
                    continue
                out.append(ev)
        return out

    def since(self, dt: datetime) -> list[LedgerEvent]:
        if not self._path.exists():
            return []
        ...
        with open(self._path, encoding="utf-8") as f:
            for raw in f:
                ...
                if ts.tzinfo is None:
                    ts = ts.replace(tzinfo=timezone.utc)
                if ts >= dt:
                    out.append(LedgerEvent.from_dict(d))
        return out
        ...
        with open(self._path, encoding="utf-8") as f:
            for raw in f:
                ...
                if d.get("id") == event_id:
                    return LedgerEvent.from_dict(d)
        return None
