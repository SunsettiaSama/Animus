...
class EmbedderBackend(Protocol):
    """Embedding backend: embed() returns a vector."""
...
class VectorBackend(Protocol):
    """Vector index: search() returns (unit_id, similarity_score) in 0~1."""
...
@dataclass
class ScoredUnit:
    """Scored memory unit for retrieval ranking."""
...
class PersonaThemeProfile:
    """Persona theme cluster profile metrics."""
...
class PersonaThemeCluster:
    """Persona recurring theme cluster for buffer metadata."""
    ...
    def to_buffer_meta(self, tick_id: str = "") -> dict:
        """Serialize cluster metadata for persona buffer."""
        return {
            "theme": self.theme,
            "tick_id": tick_id,
...
class PersonaClusterMaterial:
    """Material for persona drift from theme cluster and units."""
...
class MemoryRetriever:
    """Hybrid memory retrieval over graph store and optional vectors."""
    ...
    def persona_clusters(
        ...
        *,
        ...
        """Discover recurring persona themes from recent LTM units."""
        now = datetime.now(timezone.utc)
        candidates = [
            ScoredUnit(
                u,
    ...
    def fetch_persona_cluster(
        ...
        *,
        ...
        """Persona drift: theme + optional unit_ids for cluster material."""
        theme = theme.strip()
        if not theme:
