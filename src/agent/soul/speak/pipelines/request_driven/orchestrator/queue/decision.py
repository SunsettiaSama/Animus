    ...
    def schedule(
        ...
        if ctx.suspended_compose_count <= 0:
            get_prompt_trace().emit_event(
                session_id,
                label="queue_decision_skipped",
                payload={"token": token, "reason": "empty_suspended_queue"},
            )
            self._emit(
                session_id,
                token,
        ...
        if engine is None or engine.llm is None:
            get_prompt_trace().emit_event(
                session_id,
                label="queue_decision_skipped",
                payload={"token": token, "reason": "llm_unavailable"},
            )
            self._emit(
                session_id,
                token,
        ...
        if self._worker.status()["state"] != "running":
            get_prompt_trace().emit_event(
                session_id,
                label="queue_decision_skipped",
                payload={"token": token, "reason": "decision_worker_stopped"},
            )
            self._emit(
                session_id,
                token,
        ...
        def _job() -> None:
            ...
            get_prompt_trace().emit_event(
                session_id,
                label="queue_decision",
                payload={
                    "token": token,
                    "suspended_compose_count": ctx.suspended_compose_count,
                    "system": system,
                    "user": user,
                    "raw": raw,
                    "result": {
                        "maintain": result.maintain,
                        "thought": result.thought,
                        "reorder": list(result.reorder) if result.reorder else None,
                    },
                },
            )
