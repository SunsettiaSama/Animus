...
def test_share_reveal_by_index_returns_full_text():
    ...
    assert "今天的架构进展" in result.full_text
...
def test_prompt_composer_exposes_reveal_share():
    ...
    snap.state.expectation.to_dict.return_value = {
        "share_queue": ShareIntentQueue(
            items=[ShareIntent(topic="想聊聊天气", share_desire=ShareDesire.mild)],
        ).to_dict(),
    }
    snap.interaction.impulse_reason = ""
    ...
    assert "想聊聊天气" in revealed.full_text
...
def test_run_turn_share_state_pops_and_regenerates():
    ...
    class _LLM:
        def generate_messages(self, messages):
            call_count["n"] += 1
            if call_count["n"] == 1:
                return "[think:准备分享][state:share]"
            system = messages[0].content
            assert "【分享详情】" in system
            assert "今天的架构进展" in system
            return "[think:说][speak:今天架构有进展，值得聊聊。][state:finish]"
