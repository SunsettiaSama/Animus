...
Data 工具测试
=============
覆盖 react/action/tools/impl/data_tool.py 以及元工�?tool_search�?
...
def _jpn_parse(path: str):
    """极简 JSONPath stub：仅支持 $.key �?$.arr[*].key 形式�?""
    import re
...
class TestRegexExtract:
    ...
    def test_case_insensitive_flag(self):
        ...
        assert "共找�?2 �? in result or "共找�?3 �? in result
...
class TestToolSearch:
    ...
    def test_empty_results_message(self):
        ...
        assert "未找�? in result or "没有" in result or len(result) > 0
