...
class TestWebSearchOffline:
    ...
    def test_no_backend_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(SearXNGBackend, "is_available", lambda self: False)
        monkeypatch.setattr(TavilyBackend, "is_available", lambda self: False)
        monkeypatch.setattr(DDGBackend, "is_available", lambda self: False)
