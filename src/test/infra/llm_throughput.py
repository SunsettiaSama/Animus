"""
llm_throughput.py �?LLM 吞吐量测量引�?========================================

独立�?pytest 的纯 Python 测量核心，可以在测试和脚本中复用�?
测量指标�?  - 顺序批处理：tokens/s、延迟分�?(p50 / p95 / p99)
  - 并发吞吐：在给定并发度下的整�?tokens/s、QPS
  - 流式 TTFB（首字节延迟）、TBT（�?token 间隔时间�?  - 每次请求�?prompt_tokens / completion_tokens / latency_ms
"""
...
# ── Token 计数（复�?benchmark tokenizer）────────────────────────────────────
...
class RequestResult:
    ...
    tbt_values_ms: list[float] = field(default_factory=list)   # �?token 时间间隔
    error: str = ""
...
class ScenarioStats:
    ...
    throughput_tok_s: float       # 输出 tokens / 总耗时（wall-clock�?    qps: float                    # 请求�?/ 总耗时（wall-clock�?    error_rate: float             # 0.0 �?1.0
    wall_time_s: float
