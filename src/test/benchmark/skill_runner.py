"""
SkillRunner — smoke-gate BenchmarkRunner that validates skill action schemas
and verifies each skill can be instantiated without errors.

Gate: smoke — no LLM required, no network. Tests Pydantic validation only.
"""
...
_CASES: list[_SkillCase] = [
    _SkillCase(
        ...
        valid_args={"topic": "Python asyncio", "depth": "beginner"},
        invalid_args={"depth": "beginner"},
        description="DomainLearningSkill requires topic field",
    ),
    _SkillCase(
        name="skill_web_research_schema",
        skill_cls_path="agent.react.action.skill.web_research.WebResearchSkill",
        valid_args={"query": "latest AI developments 2026"},
        invalid_args={},
        description="WebResearchSkill requires query field",
    ),
    _SkillCase(
        name="skill_document_summary_schema",
        skill_cls_path="agent.react.action.skill.document_summary.DocumentSummarySkill",
        valid_args={"document": "This is a test document with some content."},
        invalid_args={},
        description="DocumentSummarySkill requires document field",
    ),
    _SkillCase(
...
class SkillRunner:
    ...
    Gate: smoke — only tests importability, instantiation, and Pydantic validation.
