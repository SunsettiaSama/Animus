...
class _FakeUnit:
    ...
    tier: object = field(default=None)

    def activation(self, *, now, half_life_days: float) -> float:
        return 0.5


def _retriever() -> MemoryRetriever:
    return MemoryRetriever(store=MagicMock())


def _scored(uid: str, rel: float, act: float = 0.4) -> ScoredUnit:
    u = _FakeUnit(id=uid, focus=f"f{uid}", fact=f"body{uid}")
    s = ScoredUnit(u, relevance=rel, activation=act, source="memory")  # type: ignore[arg-type]
