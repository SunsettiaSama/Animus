...
class TaoConfig:
    ...
    def _propagate_dirs(self) -> None:
        if not self.memory.medium_term.memory_dir:
            self.memory.medium_term.memory_dir = self.cache.memory_dir
        if not self.memory.long_term.memory_dir:
            self.memory.long_term.memory_dir = self.cache.memory_dir
        if not self.memory.milestone.milestone_dir:
            self.memory.milestone.milestone_dir = self.cache.milestones_dir
        if not self.persona.persona_dir:
            self.persona.persona_dir = self.cache.persona_dir
        if not self.trace.trace_dir:
            self.trace.trace_dir = self.cache.traces_dir
