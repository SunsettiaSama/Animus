...
向量库的实际载体，管理条目列表、FAISS 索引和 BGE 模型（懒加载）。
...
```python
from config.react.memory.memory_config import LongTermMemoryConfig
from react.memory.long_term import make_memory, load_store, init_empty_store
...
from react.memory.long_term.retrieve import Retriever, RetrieveRequest, RetrieveMode

retriever = Retriever(store=store, cfg=RetrieveConfig())

# 显式指定模式
result = retriever.retrieve(RetrieveRequest(
    query="上次我们讨论的任务",
    mode=RetrieveMode.HEAVY,
))
...
历史关键词覆盖中英文，例如：`"之前"` `"上次"` `"记得"` `"earlier"` `"last time"` 等。

### `PROFILE` 模式

会话启动时，若 `query` 为空，自动使用 `cfg.profile_query`（默认 `"用户习惯 偏好 常用规则 重要信息"`）执行检索，无需调用方传入 query。
...
| `memory_dir` | `"long_term_memory"` | 持久化目录 |
